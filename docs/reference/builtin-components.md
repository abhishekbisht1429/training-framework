...
| `optimizer` | Stateful resource | Owns the PyTorch optimizer, its learning-rate schedule and the fp16 gradient scaler; requires `ddp` and activates the rest of the [optimization chain](optimization.md) |
| `forward_context` | Lifecycle hook | Opens each iteration on `optimizer`: autocast and DDP `no_sync` around the forward pass; [part of the chain](optimization.md#the-chain) |
| `load_batch` | Step | Takes the next batch from `data_manager`, moves it to the device and names its parts; [generic step](generic-steps.md#load_batch) |
| `forward` | Step | Calls a model on context keys (through the DDP wrapper for the trained model); [generic step](generic-steps.md#forward) |
| `compute` | Step | Calls a function or class on context keys (losses, combinations, `torch` ops); [generic step](generic-steps.md#compute) |
| `backward` | Step | Backpropagates the loss stored under `loss_key` (default `loss`); runs after whichever step writes it; [chain](optimization.md#backward) |
...
The optional training built-ins use these configurations (`optimizer` and its
chain are in [Optimization](optimization.md); the generic steps in
[Generic steps](generic-steps.md)):
...
`optimizer` and the chain of steps it activates -- `backward`,
`freeze_gradients`, `clip_gradients`, `optimizer_step`, and the
`forward_context` hook -- have their own page: [Optimization](optimization.md).

### Generic steps

`load_batch`, `forward` and `compute` have their own page:
[Generic steps](generic-steps.md). For how to combine them into a training
iteration, see [Building an iteration](../guide/03-building-an-iteration.md).
...
[a checkpoint on disk](../guide/05-checkpoints-and-resume.md#a-checkpoint-on-disk).
...
[reading part of a checkpoint](../guide/05-checkpoints-and-resume.md#reading-part-of-a-checkpoint).
...
| `load_batch` | Step | Takes the next batch from the analysis `data_manager` and names its parts; [generic step](generic-steps.md#load_batch) |
| `forward` | Step | Calls `trained_model.model` on context keys, without gradients by default; [generic step](generic-steps.md#forward-in-an-analysis-session) |
| `compute` | Step | Shared with training; [generic step](generic-steps.md#compute) |
